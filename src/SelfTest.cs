...
namespace NetScheduler
{
    ...
    public static class SelfTest
    {
        ...
        {
            AppConfig cfg = new AppConfig(); // 默认: 周日~周四, 22:30 断, 05:00 恢复
            Check("周日21:59 有线开", WiredState.On, cfg.DesiredAuto(new DateTime(2026, 9, 6, 21, 59, 0)));
            Check("周日22:29 有线开", WiredState.On, cfg.DesiredAuto(new DateTime(2026, 9, 6, 22, 29, 0)));
            Check("周日22:30 有线关", WiredState.Off, cfg.DesiredAuto(new DateTime(2026, 9, 6, 22, 30, 0)));
            ...
            Check("白名单结束后周三22:30 有线关", WiredState.Off, wl.DesiredAuto(new DateTime(2026, 9, 9, 22, 30, 0)));
            Check("白名单日次日凌晨不开断网", WiredState.On, wl.DesiredAuto(new DateTime(2026, 9, 9, 2, 0, 0)));
            ...
            Check2("下次切换: 周一12点起 → 22:30", new DateTime(2026, 9, 7, 22, 30, 0), b);
            ...
            // 周五23:00 强制开，周末两天白名单 → 下次有效边界应为周一22:30
            ...
            Check2("周五23点起(周末白名单) → 周一22:30", new DateTime(2026, 9, 14, 22, 30, 0), b);
            ...
            CheckBool("解析: EnableWiredOnStart=false", true, !p.EnableWiredOnStart);
            CheckBool("默认: EnableWiredOnStart=true", true, new AppConfig().EnableWiredOnStart);
            CheckBool("默认: OffTime=22:30", true, new AppConfig().GetOffTime() == new TimeSpan(22, 30, 0));
            File.Delete(tmpCfg);
            Out.AppendLine();

            Out.AppendLine();
            Out.AppendLine("通过 " + _pass + " / 失败 " + _fail);

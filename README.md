...
- 🔌 **开机自启**：通过计划任务以最高权限登录自启（无 UAC 弹窗），另有 5 分钟无窗口看门狗自动拉起意外退出的进程；启动时按当前时间自动纠正状态（晚上关机、第二天开机也不会忘）
...
- 注册两个计划任务：**登录自启** + **看门狗**（每 5 分钟经 wscript 无窗口检查进程，意外退出自动拉起，**不会闪黑窗**），全程无 UAC 弹窗
- 立即启动程序，托盘出现绿/红圆点图标

升级版本时重复运行一次 `install.bat` 即可（会覆盖 exe、更新看门狗为无窗口版本，并保留你已有的 config.ini）。
...
├── install.bat         安装：复制 + 注册登录自启与 5 分钟无窗口看门狗计划任务
├── watchdog.vbs        看门狗脚本（wscript 无窗口运行，进程不在时自动拉起）
├── uninstall.bat       卸载
